...
TEMPORAL_SAMPLING = 100
RESOLUTION_RATIO = 3 # ex: res*i 2 == 1 pixel -> 2 pixels
...
def gen_new_poi_indexes(poi_indexes, movements, ratio):
    ...
    for p in range(len(poi_indexes)):
        ...
        i = poi[0]
        j = poi[1]
        # we DO NOT handle borders in this poc
        poi_neighbors = []
...
def gen_new_source_terms(ref_tensor_shape,
    ...
    if ref_tensor_shape != ns_tensor_n.shape:
            raise ValueError("different tensor shapes.")
    ...
    for ps in range(len(new_poi_idx)):
        ...
        split_data = data[timestep] / RESOLUTION_RATIO * DEBUG_RATIO
        #print(f"{split_data = }")
        for poi_i, poi_j in poi_set:
            ns_tensor_n[0, poi_i, poi_j] = split_data

    #print(f"{ns_tensor_n = }")
    temp = ns_tensor_n[0, 1001, 190]
    print(f"{temp = }")
...
if len(sys.argv) != 3:
    print('missing parameter: <geojson_file> <geotiff_file>')
    sys.exit(1)

(_, geojson_file, geotiff_file) = sys.argv
...
print(f"Coordonnées du pixel ({i},{j}): x={x}, y={y}")

tx = 1038046.5
ty = 6869606.5
idx = spatial_to_index_coords(spatial_coords, tx, ty)
